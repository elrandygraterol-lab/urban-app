...
export default function VerifyEmailScreen() {
  ...
  const handleVerifyEmail = async () => {
    ...
    setIsLoading(true);
    try {
      await authAPI.verifyEmail(trimmedCode, email);
      showStatus(
        'success',
  ...
  const handleResendCode = async () => {
    ...
    try {
      ...
      showToast('Se ha enviado un nuevo código a tu email', 'success');
    } catch (error: any) {
      const errorMessage =
        error?.response?.data?.error?.message ||
        error?.message ||
          ...
          <View style={styles.cardContainer}>
            <View style={styles.formContainer}>
              ...
              <View style={styles.formHeader}>
                ...
                <Text style={styles.formTitle}>Verificar email</Text>
              ...
              >
                {isLoading ? (
                  ...
                  <Text style={styles.primaryBtnText}>Verificar email</Text>
                )}
              </TouchableOpacity>

              <TouchableOpacity
                style={[styles.linkBtn, resendCooldown > 0 && styles.linkBtnDisabled]}
                onPress={handleResendCode}
                disabled={isLoading || resendCooldown > 0}
              >
                <Ionicons name="refresh-outline" size={16} color="#2FB908" />
